...
function Hide()
	...
	Debounce = true
	RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " ..inputKey01.."!", Duration = 3})
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[inputKey01] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
